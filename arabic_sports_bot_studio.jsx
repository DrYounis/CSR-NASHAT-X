...
export default function ArabicSportsBotStudio() {
  ...
  const generatePosts = async () => {
    ...
    try {
      ...
      const systemPrompt = `أنت خبير في إنشاء محتوى رياضي عربي متخصص لمنصة X (تويتر). 
مهمتك هي:
1. البحث في معلوماتك عن الموضوع المطلوب
2. استخلاص أفكار قيّمة وحقيقية
3. تحويلها إلى منشورات رياضية عربية احترافية

قواعد صارمة:
- كل منشور يجب أن يكون بين 200-280 حرف
- المحتوى باللغة العربية الفصحى المبسطة
- معلومات صحيحة وموثوقة
- مناسب لجمهور الخليج العربي
- أسلوب ${POST_STYLES.find(s => s.id === selectedStyle)?.label}
- فئة المحتوى: ${selectedCategory}
- السياق: ${sourceContextMap[selectedSource]}
${includeHashtags ? "- أضف 3-4 هاشتاقات عربية ذات صلة في نهاية كل منشور" : "- بدون هاشتاقات"}
${includeEmoji ? "- استخدم إيموجي مناسبة (2-3 لكل منشور)" : "- بدون إيموجي"}

أجب بصيغة JSON بالضبط:
{
  "research_insights": ["فكرة 1 مستخرجة من البحث", "فكرة 2", "فكرة 3"],
  "posts": [
    {
      "id": 1,
      "text": "نص المنشور الكامل",
      "hook": "ما الذي يجذب الانتباه في هذا المنشور",
      "best_time": "صباح أو مساء أو أي وقت",
      "engagement_tip": "نصيحة لزيادة التفاعل"
    }
  ]
}`;

      const userPrompt = `ابحث عن المعلومات المتاحة لديك حول: "${topic}"
في سياق: ${sourceContextMap[selectedSource]}

ثم أنشئ ${postCount} منشور رياضي عربي احترافي لمنصة X بأسلوب ${POST_STYLES.find(s => s.id === selectedStyle)?.label} لفئة ${selectedCategory}.

تأكد أن المعلومات دقيقة وحديثة ومفيدة للجمهور العربي الرياضي.`;
      ...
      setError("حدث خطأ أثناء توليد المحتوى. تأكد من الاتصال وأعد المحاولة.");
  ...
  const savePost = (post) => {
    ...
    setSavedPosts(prev => [toSave, ...prev]);
  };

  const removePost = (id) => {
    setSavedPosts(prev => prev.filter(p => p.id !== id || p.savedAt));
  };
